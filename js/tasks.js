  ...
  return `
    <div class="task-card" data-id="${task.id}">
      ...
      <div class="task-meta">
        ...
        <label class="deadline-btn-wrap">
          <span class="deadline-btn ${dlClass}" data-id="${task.id}">
            ${task.deadline ? `📅 ${dlLabel}` : "📅 納期を設定"}
          </span>
          <input type="date" class="deadline-input-hidden" data-id="${task.id}" value="${task.deadline || ""}">
        </label>
      </div>
      ${subtasksHTML}
    </div>
...
function attachTaskEvents() {
  ...
  document.querySelectorAll(".deadline-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const wrap = btn.closest(".deadline-btn-wrap");
      const input = wrap.querySelector(".deadline-input-hidden");
  ...
  // 納期インライン編集
  document.querySelectorAll(".deadline-input-hidden").forEach(el => {
    el.addEventListener("change", async () => {
      const id = el.dataset.id;
      const task = tasks.find(t => t.id === id);
      if (!task) return;
      task.deadline = el.value;
      await updateDoc(doc(db, "tasks_v2", id), { deadline: el.value });
      renderTasks(document.getElementById("sortSelect")?.value || "status");
      showToast("納期を更新しました");
    });
  });
