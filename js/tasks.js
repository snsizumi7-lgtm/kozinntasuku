...
import { todayStr, countBusinessDays, isHoliday } from "./holidays.js";

export const STATUSES = [
  "要対応","見積もり作成","着手","対応中","確認中","修正対応中","納品","請求書","完了"
];
...
function deadlineLabel(dateStr) {
  ...
  const today = todayStr();
  const bdays = countBusinessDays(today, dateStr);
  if (bdays < 0) return `${dateStr}（${Math.abs(bdays)}営業日超過）`;
  if (bdays === 0) return `${dateStr}（本日）`;
  return `${dateStr}（残${bdays}営業日）`;
}
      ...
      ${subs.map(s => {
        ...
        return `
          <div class="subtask-row">
            ...
            ${s.assignee ? `<span class="subtask-assignee">${esc(s.assignee)}</span>` : ""}
            ${s.dueDate ? `<span class="subtask-due ${sDlClass}">${s.dueDate}</span>` : ""}
          </div>
        `;
      }).join("")}
      ${subs.length > 0 ? `
  ...
  return `
    <div class="task-card" data-id="${task.id}">
      ...
      <div class="task-meta">
        ...
        ${task.deadline ? `<span class="deadline-badge ${dlClass}">⏰ ${dlLabel}</span>` : ""}
      </div>
      ${subtasksHTML}
    </div>
  `;
...
function attachTaskEvents() {
  // Status badge click → next status
...
export function openAddModal() {
  ...
  document.getElementById("taskStatus").value = "要対応";
...
function openEditModal(id) {
  ...
  document.getElementById("taskStatus").value = task.status || "要対応";
...
export function addSubtaskRow(sub = null) {
  ...
  row.innerHTML = `
    ...
    <select data-field="assignee" data-id="${subId}">
      <option value="">担当者</option>
      ${assignees.map(a => `<option value="${a}" ${sub?.assignee === a ? "selected" : ""}>${a}</option>`).join("")}
    </select>
...
export async function saveTask() {
  ...
  document.querySelectorAll(".subtask-editor-row").forEach(row => {
    ...
    const assignee = row.querySelector("[data-field='assignee']").value;
    ...
    subtasks.push({ id, text, assignee, dueDate, done });
  });
